...
public class PropertyReport extends Entity implements iEntity {
    ...
    private List<Unit> multi_units;
    private List<Repair> repairs;
    private Address address;
    private AddressController ac;

    public PropertyReport(Context context, long address_id, String property_name, String style, int sq_ft, int lot_size, int is_multi_unit, int year_built, double hoa_fees, int is_occupied,
    ...
    public PropertyReport(Context context, long id, long address_id, String property_name, String style, int sq_ft, int lot_size, int is_multi_unit, int year_built, double hoa_fees, int is_occupied,
    ...
    public PropertyReport(Context context){this.context = context; }
    ...
    public List<Unit> getMulti_units() {
        if(multi_units == null || multi_units.isEmpty()){
            fetchUnits();
        }
        return multi_units;
    }
    ...
    private void fetchUnits(){
        uc = new UnitController(this.context);
        Uri uri = Uri.parse("content://" + uc._provider.getAuthority() + "/" + uc._provider.get_table() + "/" + _id);
        this.multi_units = uc.getByPropertyId(uri);
    }

    private void fetchRepairs(){
        rc = new RepairController(this.context);
        Uri uri = Uri.parse("content://" + rc._provider.getAuthority() + "/" + rc._provider.get_table() + "/" + _id);
        this.repairs = rc.getByPropertyId(uri);
        rtc = new RepairTypeController(this.context);
        for (Repair r : this.repairs){
            uri = Uri.parse("content://" + rtc._provider.getAuthority() + "/" + rtc._provider.get_table() + "/" + r.getRepair_type_id());
            r.setRepair_type((RepairType)rtc.getById(uri));
        }
    }

    private void fetchAddress(){
        ac = new AddressController(this.context);
        Uri uri = Uri.parse("content://" + ac._provider.getAuthority() + "/" + ac._provider.get_table() + "/" + address_id);
        this.address = (Address)ac.getById(uri);
    }
